...
function findProtagonist(
  ...
  if (name) {
    return (
      chars.find((c) => c.name === name || c.aliases?.includes(name)) || null
    );
  }
  return chars[0] || null;
}

export function extractProtagonistAttitude(
  memory: WriterMemory,
...
export function extractCoreRelationships(
  memory: WriterMemory,
  protagonistName?: string,
): string {
  const protagonist = findProtagonist(memory, protagonistName);
...
export function updateSynopsisElement(
  element: keyof SynopsisState,
  value: string,
): boolean {
  const memory = loadMemory();
  if (!memory) return false;
...
export function formatFullSynopsis(
  ...
  const charList = chars
    .map((c) => `- **${c.name}**: ${c.attitude || c.arc || "설명 없음"}`)
    .join("\n");
  const emotionFlow =
    memory.scenes
...
export function getSynopsisChecklist(memory: WriterMemory): ChecklistItem[] {
    ...
    status:
      hasArc && hasAttitude
        ...
        : hasArc || hasAttitude
          ...
          : "missing",
    source: protagonist
      ? `캐릭터 '${protagonist.name}'에서 추출`
      : "주인공 없음",
    suggestion:
      hasArc && hasAttitude
        ? ""
        : 'char update <name> arc "..." attitude "..."',
  });

  // 2. Core Relationships
  const relCount = protagonist
  ...
  checklist.push({
    ...
    suggestion: hasGenreContrast
      ? ""
      : 'synopsis update genreVsRealEmotion "..."',
  });
