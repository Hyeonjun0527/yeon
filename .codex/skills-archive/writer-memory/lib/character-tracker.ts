...
export function validateDialogue(
  ...
  const speechCheck = {
    ...
    detail:
      detected === character.speechLevel
        ? "말투 일치"
        : `기대: ${character.speechLevel}, 감지: ${detected}`,
  };
  ...
  if (character.keywords.length > 0) {
    ...
    keywordCheck.detail = hasKeyword
      ? "특징 키워드 포함"
      : `키워드 미포함: ${character.keywords.slice(0, 2).join(", ")}`;
  }

  const failCount = [toneCheck, speechCheck, keywordCheck].filter(
    (c) => !c.passed,
  ).length;
  const status: "PASS" | "WARN" | "FAIL" =
    failCount === 0 ? "PASS" : failCount >= 2 ? "FAIL" : "WARN";

  const suggestions: string[] = [];
  if (!toneCheck.passed) suggestions.push(`톤 조정 필요`);
  if (!speechCheck.passed)
    suggestions.push(`${character.speechLevel}로 말투 수정`);
