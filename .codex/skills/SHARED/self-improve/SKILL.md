...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   a. **No-regression check**: candidate score must improve or hold even vs `best_score`, respecting `benchmark_direction` (`higher_is_better`: score >= best*score; `lower_is_better`: score <= best_score)
   b. **Merge** via `oh-my-claudecode:git-master`: `git merge experiment/round*{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`c. **Re-benchmark** on merged state to confirm improvement
