...
- 백그라운드 위임은 기다리는 동안 진행할 독립 작업이 실제로 남아 있을 때만 사용한다. 다음 판단이 그 결과에 즉시 의존하면 foreground 실행을 우선한다.
- 진행 중인 작업이 장시간 대기 상태로 들어가면 타임아웃까지 무작정 기다리지 말고, 현재 완료 범위와 남은 대기 이유를 먼저 공유한다.
- 백그라운드 탐색/조사 결과가 꼭 필요한데 `<system-reminder>`가 합리적인 시간 안에 오지 않으면, 알림 경로가 degraded 된 것으로 보고 현재 상태를 공유한 뒤 로컬 `grep`/`read`/`ast-grep` 탐색이나 foreground 재실행으로 전환한다.
...
- 외부 에이전트 결과 알림만 기다리며 응답을 끝내는 상태를 반복하지 않는다. 알림이 없으면 멈춘 것이 아니라 fallback 할 시점으로 본다.
