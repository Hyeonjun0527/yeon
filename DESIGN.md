# Design System — Yeon (Inspired by Linear)

## 0. Yeon Project Token Overrides ⚠️ ALWAYS USE THESE FIRST

> This project uses CSS custom properties defined in `apps/web/src/app/globals.css`.
> **Always use these tokens via Tailwind classes or CSS variables — never hardcode hex values.**
> The Linear-inspired sections below describe design philosophy and component patterns only.

### CSS Variables (`.app-theme`)

```
Background & Surfaces
  --bg:          #09090b   → bg-bg, background
  --surface:     #111113   → bg-surface
  --surface2:    #18181b   → bg-surface-2
  --surface3:    #222225   → bg-surface-3
  --surface4:    #2c2c30   → bg-surface-4

Borders
  --border:        #27272a   → border-border
  --border-light:  #333338   → border-border-light

Text
  --text:           #fafafa   → text-text
  --text-secondary: #a1a1aa   → text-text-secondary
  --text-dim:       #52525b   → text-text-dim

Accent (Indigo-Violet)
  --accent:         #818cf8   → text-accent, bg-accent
  --accent-hover:   #6366f1
  --accent-dim:     rgba(129,140,248,0.1)  → bg-accent-dim
  --accent-border:  rgba(129,140,248,0.2)  → border-accent-border
...
- **Primary font**: `"Noto Sans KR"` (Korean-first), then `-apple-system`, `BlinkMacSystemFont`, `"Segoe UI"`, `sans-serif`
- **Monospace**: `ui-monospace`, `SFMono-Regular`, `Menlo`
- Use `font-mono` Tailwind class for monospace elements (timestamps, code, IDs)

### Tailwind Class Conventions

```
Surfaces:    bg-surface-2, bg-surface-3, bg-surface-4
...
- ❌ Hardcode hex colors like `#818cf8` directly in JSX/CSS — use `var(--accent)` or `text-accent`
- ❌ Use `bg-indigo-500` or Tailwind color utilities for brand colors — use custom tokens
- ❌ Mix light-mode Tailwind utilities (`bg-white`, `text-gray-900`) in `.app-theme` components
- ❌ Use `text-white` — use `text-text` instead
