...
export function useSpaceSettings(spaceId: string | null) {
  ...
  const loadFields = useCallback(async (tabId: string) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "필드를 불러오지 못했습니다.");
    } finally {
  ...
  const reorderTabs = useCallback(async (fromIdx: number, toIdx: number) => {
    if (!spaceId) return;
    ...
    try {
      ...
      setTabs(tabs);
      setError(e instanceof Error ? e.message : "탭 순서를 변경하지 못했습니다.");
    }
  }, [spaceId, tabs]);

  const createField = useCallback(async (tabId: string, name: string, fieldType: FieldType) => {
    if (!spaceId) return;
    setError(null);
    try {
      ...
      setError(e instanceof Error ? e.message : "필드를 생성하지 못했습니다.");
    }
  }, [spaceId]);
  ...
  const deleteField = useCallback(async (fieldId: string) => {
    ...
    try {
      ...
      setFields(prev);
      setError(e instanceof Error ? e.message : "필드를 삭제하지 못했습니다.");
    }
  }, [spaceId, fields]);

  const reorderFields = useCallback(async (tabId: string, fromIdx: number, toIdx: number) => {
    if (!spaceId) return;
    ...
    try {
      ...
      setFields(fields);
      setError(e instanceof Error ? e.message : "필드 순서를 변경하지 못했습니다.");
    }
  }, [spaceId, fields]);
