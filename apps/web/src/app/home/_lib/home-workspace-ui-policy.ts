...
export type HomeWorkspaceSurface =
  | "space-gate"
  | "empty"
  | "recording"
  | "workspace";

export type HomeWorkspaceUiPolicy = {
  surface: HomeWorkspaceSurface;
...
type HomeWorkspaceUiPolicyParams = {
  spacesLoading: boolean;
  spaceCount: number;
  viewStateKind: HomeViewState["kind"];
  hasSelectedMember: boolean;
  selectedRecordStatus: CounselingRecordStatus | null;
};

export function getHomeWorkspaceUiPolicy(
  params: HomeWorkspaceUiPolicyParams,
): HomeWorkspaceUiPolicy {
