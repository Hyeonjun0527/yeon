...
- 빌드/타입 에러가 **내가 수정하지 않은 파일**에서 발생하면, 다른 에이전트가 작업 중인 WIP 파일일 수 있다.
- 이 경우 직접 수정하기 전에 에러 발생 파일이 현재 다른 작업 브랜치에 포함된 것인지 확인한다.
- 다른 에이전트의 WIP로 인한 에러라면 직접 수정하지 말고 사용자에게 보고한다.
