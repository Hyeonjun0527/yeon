...
- `develop`은 QA 기준 브랜치, `main`은 운영 기준 브랜치로 사용한다.
...
- 단, 매우 많은 에이전트가 동시에 같은 저장소에서 작업 중이고 unrelated 변경이 계속 생기는 상황에서는 다른 에이전트 변경을 존중한다.
- 위 상황에서는 갑자기 수정된 파일을 함부로 되돌리거나 덮어쓰지 말고, 본인 작업 범위 파일만 `git add <path>`로 분리해 빠르게 커밋한다.
