...
- 현재 `YEON`의 우선 제품 가설은 단순 `상담 요약 기능`이 아니라 `상담 기록 워크스페이스`다.
- 기능 수를 늘리기보다 `녹음/업로드 -> 고품질 STT -> 원문 전체 열람 -> 구조화 요약 -> 원문 기반 AI 채팅 -> 학생별 누적 기록` 흐름을 먼저 닫는다.
- 기존의 넓은 `AI 학생관리 CRM` 아이디어와 충돌하면, 당분간은 상담 기록 워크스페이스 품질을 우선한다.
- 원문 텍스트는 부가 자료가 아니라 신뢰의 source of truth다. 요약만 보여주는 방향으로 후퇴하지 않는다.
- STT 품질, 원문 누락 여부, 긴 상담 처리 안정성은 장식 기능보다 우선한다.
- AI 요약은 감성적 문장보다 실무형 구조를 따른다.
  - `핵심 상담 내용`
  - `학생 문제 포인트`
  - `보호자 요청사항`
  - `다음 액션`
- `다음 액션`은 판단이 아니라 추천이어야 하며, 사용자가 수정/삭제/추가할 수 있어야 한다.
- 상담 AI 채팅은 일반 지식형 챗봇이 아니라 `현재 선택한 상담 원문`을 근거로 답하는 문서 탐색 보조 도구로 설계한다.
- 상담 관련 기본 화면 구조는 `왼쪽 상담 리스트 / 가운데 원문 중심 본문 / 오른쪽 AI 채팅`을 기준으로 잡는다.
- 왼쪽 리스트는 `학생명, 상담 일시, 녹음 길이, 처리 상태`를 빠르게 선택하게 해야 한다.
- 가운데 본문은 `원문 전체 텍스트`를 가장 넓고 읽기 쉬운 영역으로 확보하고, 상단에 구조화 요약과 메타 정보를 둔다.
- 오른쪽 채팅은 `보호자 요청사항만 뽑아줘`, `다음 상담 때 확인할 점 정리해줘` 같은 원문 기반 질의를 빠르게 수행하게 한다.
- 상담 기능 구현 시 과한 감정 판정, 근거 없는 학생 평가, 자동 확정 판단은 피한다.
- 학생별 기록 누적, 검색, 접근 권한 관리는 후순위 장식 기능이 아니라 제품 신뢰성의 일부로 본다.
...
- 이 저장소는 최대 3개의 worktree를 사용한다.
  - **A**: 기존 레포 (메인 작업 디렉터리, `/home/osuma/coding_stuffs/yeon`)
...
- 이미 작업 중인 feat 브랜치가 있고 사용자가 그 위에서 계속 작업하라고 지시하면 해당 브랜치에서 이어간다.
- 다른 에이전트가 이미 작업 중인 브랜치에 추가 작업이 필요하면, 그 브랜치 위에 함께 작업을 올린다.
- worktree를 3개 초과로 만드는 것은 금지한다.

### 개발 완료 필수 원칙

- 개발 코드 작업을 마치면 반드시 아래 순서를 빠짐없이 일사천리로 수행한다. 이것은 선택이 아닌 필수 원칙이다. 개발 병목을 최소화하기 위해 중간에 멈추지 않는다.
  1. 테스트 (lint, typecheck, build 등 해당 workspace의 검증 스크립트)
  2. 커밋
  3. push
  4. Pull Request 생성 (PR이 이미 열려 있으면 기존 PR에 push)
  5. `develop` 브랜치에 머지 (`gh pr merge` 사용)
- 이미 같은 브랜치에서 다른 에이전트가 작업한 이력이 있다면, 그 브랜치 위에 작업을 올려서 PR을 생성하고 머지까지 완료한다.
- 이 전체 흐름은 사용자 확인 없이 자동으로 진행한다. 실패한 단계가 있으면 그 시점에서 사용자에게 보고한다.
