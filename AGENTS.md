...
- 사용자가 커밋을 요청하면 아래 순서를 반드시 그대로 따른다.
  ...
  4. 커밋 후 `yarn lint`
  5. 커밋 후 `yarn prettier`
  6. 커밋 후 `yarn typecheck`
- 현재 저장소는 다중 에이전트 동시 작업 상황을 기본으로 보고, 다른 에이전트 변경과 섞이지 않게 자기 파일만 staging 한다.
- 이 저장소가 `pnpm` 모노레포라 실제 `yarn` 스크립트명이 없으면 같은 순서를 유지하는 동등 명령으로 치환한다.
...
- 커밋 후 검증이 실패하면 실패 내용을 즉시 공유하고, 같은 브랜치에서 후속 수정 커밋으로 이어간다.
...
- 다중 에이전트 동시 작업 중 갑자기 파일 수정이 보이면 다른 에이전트 변경으로 간주하고 먼저 존중한다.
